...
public class SwerveModule {
    ...
    private Rotation2d getAngle() {
        ...
        return Rotation2d.fromDegrees(
                Conversions.talonToDegrees(m_angleMotor.getPosition().getValueAsDouble(), Constants.ANGLE_GEAR_RATIO));
    }
    ...
    public double getCANcoderWithOffset() {
        return m_angleEncoder.getAbsolutePosition().getValueAsDouble() - m_angleOffset.getDegrees();
    }

    private Rotation2d waitForCANcoder() {
        /* wait for up to 250ms for a new CANcoder position */
        return Rotation2d.fromRotations(m_angleEncoder.getAbsolutePosition().waitForUpdate(250).getValueAsDouble());
    }
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(
                Conversions.talonToMPS(m_driveMotor.getVelocity().getValueAsDouble(), Constants.WHEEL_CIRCUMFERENCE,
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                Conversions.talonToMeters(m_driveMotor.getPosition().getValueAsDouble(), Constants.WHEEL_CIRCUMFERENCE,
                        Constants.DRIVE_GEAR_RATIO),
                getAngle());
    }
}